$(function () {
	...
	app.controller('mpscontroller', ["$scope", "$http", "$interval", "$window", function ($scope, $http, $interval, $window) {
		...
		$scope.setVersion = function () {
			var v = prompt($scope.defect.VERSION);
			if (v) {
		...
		$scope.currMessage = "";
		$scope.messrows = 1;
		$scope.messageSubmit = function (clr, img) {
			var trim = $scope.currMessage.trim();
			if (trim !== "") {
				if (img) {
					$scope.commented = true;
					$scope.commented_txt = trim;
					$scope.commented_img = img;
				}
				$scope.addComment(trim, false, clr);
				$scope.currMessage = "";
				$scope.messrows = 1;
			}
		};
		...
		$scope.addComment = function (txt, apply, clr) {
			var d = new Date();
			if (clr !== "") {
				clr = " clr='" + clr + "' ";
			}
			$scope.defect.DESCR = "<task-message" + clr + " userid='" + userID() + "' user='" + userLogin() + "' time='" +
		...
		$scope.adddesc = function (text, img, clr) {
